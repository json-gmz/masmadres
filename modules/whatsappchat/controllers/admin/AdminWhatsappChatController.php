...
class AdminWhatsappChatController extends ModuleAdminController
{
    ...
    public function printAudience($value, $config)
    {
        ...
        if ($config['customers'] == '' || $config['customers'] == 'all') {
            ...
            $text = version_compare(_PS_VERSION_, '1.6', '>=') ? Tools::rtrimString($text, ',') : rtrim($text, ',').'</li>';
        }
    ...
    public function renderForm()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.6', '<')) {
            $category_link = (Configuration::get('PS_SSL_ENABLED') ? 'https://' : 'http://').$_SERVER['HTTP_HOST'].__PS_BASE_URI__.$category->link_rewrite[$this->context->language->id];
        } else {
        ...
        $this->context->smarty->assign(array(
            ...
            'whatsappchat_categorypage' => $category_link.'?show_whatsapp=1',
