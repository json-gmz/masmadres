...
class CustomerPersisterCore
{
    ...
    public function save(Customer $customer, $clearTextPassword, $newPassword = '', $phone_main = '', $service, $product_service, $passwordRequired = true, $fileone,  $filetwo, $filetree, $filefour)
    {
        if ($customer->id) {
            return $this->update($customer, $clearTextPassword, $newPassword, $phone_main, $service, $product_service, $passwordRequired);
        } else {
            return $this->create($customer, $clearTextPassword, $phone_main, $service, $product_service, $fileone, $filetwo, $filetree, $filefour);
        }
    }

    private function update(Customer $customer, $clearTextPassword, $newPassword, $phone_main, $service, $product_service, $passwordRequired = true)
    {
        ...
        $customer->phone_main = $phone_main;
        $customer->service = $service;
        $customer->product_service = $product_service;
    ...
    private function create(Customer $customer, $clearTextPassword, $phone_main, $service, $product_service, $fileone, $filetwo, $filetree, $filefour)
    {
        ...
        if($fileone != ''){
            ...
            $destination =  './momfiles/'.($customer_id.'_'.'dni'.'.'.$extension);

            move_uploaded_file($fileone, $destination);
        }

        if($filetwo != ''){
            
            $filename = $_FILES['filetwo']['name'];
            $extension = pathinfo($filename, PATHINFO_EXTENSION);
            $customer_id = $customer->id;
            $destination =  './momfiles/'.($customer_id.'_'.'criminal'.'.'.$extension);

            move_uploaded_file($filetwo, $destination);
        }

        if($filetree != ''){
            
            $filename = $_FILES['filetree']['name'];
            $extension = pathinfo($filename, PATHINFO_EXTENSION);
            $customer_id = $customer->id;
            $destination =  './momfiles/'.($customer_id.'_'.'sons'.'.'.$extension);

            move_uploaded_file($filetree, $destination);
        }

        if($filefour != ''){
            
            $filename = $_FILES['filefour']['name'];
            $extension = pathinfo($filename, PATHINFO_EXTENSION);
            $customer_id = $customer->id;
            $destination =  './momfiles/'.($customer_id.'_'.'video'.'.'.$extension);

            move_uploaded_file($filefour, $destination);
        }
    ...
    private function sendConfirmationMail(Customer $customer)
    {
        ...
        $message_customer = "";
        if ( $customer->service != 0 && $customer->service > 0 ) {
            $message_customer = "En los pr&oacute;ximos d&iacute;as nos pondremos en contacto contigo para seguir con el proceso de registro.";
        }
