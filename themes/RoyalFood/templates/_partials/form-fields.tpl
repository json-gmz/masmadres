    ...
    <div class="col-md-6{if ($field.type === 'radio-buttons')} form-control-valign{/if}">
      ...
      {elseif $field.type === 'date'}
        ...
        {block name='form_field_item_date'}
          {assign var=date value="/"|explode:$field.value}
          <input name="{$field.name}" class="form-control" type="date" value="{$date[2]}-{$date[1]}-{$date[0]}" placeholder="{if isset($field.availableValues.placeholder)}{$field.availableValues.placeholder}{/if}">
          {if isset($field.availableValues.comment)}
            <small>
              {l s="Para nosotros es importante celebrar tu cumpleaños" d='Shop.Theme.Customeraccount'}
            </small>
          {/if}
        {/block}
      ...
      {else}
        ...
        {block name='form_field_item_other'}
          <input
            ...
            type="{$field.type}"
            value="{$field.value}"
            {if isset($field.availableValues.placeholder)}placeholder="{$field.availableValues.placeholder} - {$field.availableValues.comment}"{/if}
